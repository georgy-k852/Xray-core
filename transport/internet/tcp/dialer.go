...
func Dial(ctx context.Context, dest net.Destination, streamSettings *internet.MemoryStreamConfig) (stat.Connection, error) {
	...
	if config := tls.ConfigFromStreamSettings(streamSettings); config != nil {
		...
		if r, ok := tlsConfig.Rand.(*tls.RandCarrier); ok && IsFromMitm(r.ServerNameToVerify) {
			r.ServerNameToVerify = session.MitmServerNameFromContext(ctx)
		}
		if fingerprint := tls.GetFingerprint(config.Fingerprint); fingerprint != nil {
			conn = tls.UClient(conn, tlsConfig, fingerprint)
			if len(tlsConfig.NextProtos) == 1 && (tlsConfig.NextProtos[0] == "http/1.1" ||
				(IsFromMitm(tlsConfig.NextProtos[0]) && session.MitmAlpn11FromContext(ctx))) {
